...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb=None):
        h = x
        h = self.norm1(h)
        # BN affine = False will not use nonlinearity
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if self.temb_ch is not None:
            ...
            self.temb = torch.nn.UpsamplingNearest2d(resolution)
    ...
    def forward(self, x, t):
        ...
        if self.temb_ch is not None:
            ...
            temb = self.temb(temb.view(x.shape[0], self.ch, 1, 1))
            x = torch.concat([x, temb], dim=1)
        ...
        # end
        h = self.norm_out(h)  # keras no norm_out ddim has
...
if __name__ == "__main__":
    ...
    model = Model(new_config).to("cuda")
    inputs = torch.ones((2, 3, new_config.data.image_size, new_config.data.image_size)).to("cuda")
    diffusion_times = torch.ones(2).to("cuda")
    outputs = model(inputs, diffusion_times)
    print(outputs.shape)
    assert outputs.size() == (2, 3, new_config.data.image_size, new_config.data.image_size)
